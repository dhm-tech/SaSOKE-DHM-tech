...
def getCheckpointCallback(cfg, logger=None, **kwargs):
    ...
    checkpointParams = {
        ...
        'monitor': "loss_total",  # Monitor loss to save best checkpoints
        'mode': "min",  # Save checkpoints with minimum loss
        'every_n_epochs': 10,  # Save every 10 epochs (to save disk space)
        'save_top_k': 3,  # Keep only top 3 best checkpoints
        'save_last': True,  # Also save last checkpoint
        'save_on_train_epoch_end': True  # Save after each training epoch
    }
    callbacks.append(ModelCheckpoint(**checkpointParams))
