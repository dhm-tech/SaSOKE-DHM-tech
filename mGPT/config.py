...
def resume_config(cfg: OmegaConf):
    ...
    if cfg.TRAIN.RESUME:
        ...
        if os.path.exists(resume):
            # Check if resume is a checkpoint file (.ckpt) or a directory
            if resume.endswith('.ckpt'):
                # Direct checkpoint file path
                cfg.TRAIN.PRETRAINED = resume
                # Skip wandb resume for direct checkpoint files
            else:
                # Directory path (original behavior)
                # Checkpoints
                cfg.TRAIN.PRETRAINED = pjoin(resume, "checkpoints", "last.ckpt")
                # Wandb (only if wandb directory exists)
                wandb_path = pjoin(resume, "wandb", "latest-run")
                if os.path.exists(wandb_path):
                    wandb_files = os.listdir(wandb_path)
